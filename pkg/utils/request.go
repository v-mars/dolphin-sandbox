...
func Request(url, method string, param interface{}, client *http.Client, reqParam ReqParam,
	...
	if ok {
		...
		if reqParam.Headers[conf.HeaderEncTag] == "yes" {
			log.Debugf("request url[%s] body param enc is start", url)
			bdata, err := json.Marshal(&param)
			if err != nil {
				log.Errorf("request url[%s] body param enc json marshal is err %s", url, err)
