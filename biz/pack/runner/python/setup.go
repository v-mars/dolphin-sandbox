...
import (
	...
	python_dependencies "dolphin-sandbox/biz/pack/runner/python/dependencies"
)
...
func InstallDependencies(requirements string) error {
	...
	return runner.WithTempDir("/", []string{}, func(root_path string) error {
		...
		if err != nil {
			log.Println("failed to create requirements.txt")
			return nil
		}

		// install dependencies
		pipMirrorURL := static.GetSandboxGlobalConfigurations().PythonPipMirrorURL
		...
		if err != nil {
			log.Println("failed to get stdout pipe of pip3")
			return err
		}
		defer reader.Close()
		...
		err = cmd.Start()
		if err != nil {
			log.Println("failed to start pip3")
			return err
		}
		...
		if err != nil {
			log.Println("failed to wait for the command to complete")
			return err
		}
...
func RefreshDependencies() []types.Dependency {
	log.Println("updating python dependencies...")
	dependencies := static.GetRunnerDependencies()
	err := InstallDependencies(dependencies.PythonRequirements)
	if err != nil {
		log.Println("failed to install python dependencies: %v", err)
		return nil
	}
	log.Println("python dependencies updated")
	return python_dependencies.ListDependencies()
}
