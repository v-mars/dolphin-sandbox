...
func (l *Logger) CtxLogf(level hlog.Level, ctx context.Context, format string, kvs ...interface{}) {
	...
	switch level {
	case hlog.LevelTrace, hlog.LevelDebug:
		logger.Debug().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	case hlog.LevelInfo:
		logger.Info().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	case hlog.LevelNotice, hlog.LevelWarn:
		logger.Warn().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	case hlog.LevelError:
		logger.Error().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	case hlog.LevelFatal:
		logger.Fatal().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	default:
		logger.Warn().Ctx(ctx).Str(LogIDKey, logId).Msg(fmt.Sprintf(format, kvs...))
	}
}
